...
struct WolfEmuSigSplicePass : public Pass
{
    ...
    std::map<std::set<std::pair<RTLIL::SigSpec, int>>, RTLIL::SigSpec> wire_sig_bit_cluster;    // 用于聚类
    std::map<std::tuple<RTLIL::SigSpec, int, RTLIL::SigBit>, RTLIL::SigBit> sig_bit_update_map; // 用于存储信号与线的映射 <old_sig_bit, new_sig_bit>
    ...
    void __debug_print_sigspec(const RTLIL::SigSpec &sig_spec)
    {
        log("SigSpec(total_bits=%d): ", sig_spec.size());

        bool first = true;
        for (const auto &chunk : sig_spec.chunks())
        {
            if (!first)
                log(" | ");
            first = false;
            ...
            { // 常量
                ...
                for (RTLIL::State s : chunk.data)
                    bits += s == RTLIL::State::S0 ? '0' : s == RTLIL::State::S1 ? '1'
                                                      : s == RTLIL::State::Sx   ? 'x'
                                                      : s == RTLIL::State::Sz   ? 'z'
                                                                                : '?';
                log("%s", bits.c_str());
    ...
    void __debug_print_sigbit(const RTLIL::SigBit &sig_bit)
    {
        if (sig_bit.is_wire())
        {
            log("SigBit(wire=%s, offset=%d)\n", log_id(sig_bit.wire), sig_bit.offset);
        }
        else
        {
            log("SigBit(state=%c)\n", sig_bit.data == RTLIL::State::S0 ? '0' : sig_bit.data == RTLIL::State::S1 ? '1'
                                                                                                          : sig_bit.data == RTLIL::State::Sx ? 'x'
                                                                                                          : sig_bit.data == RTLIL::State::Sz ? 'z'
                                                                                                          : '?');
        }
    }
    ...
    void __debug_print_wire_sig_bit_cluster()
    {
        // 遍历 wire_sig_bit_cluster，打印每个 SigSpec 的信息
        for (const auto &entry : wire_sig_bit_cluster)
        {
            const auto &sig_cluster = entry.first;
            auto tmp_sig_spec = entry.second;
            log("Wire-SigBit Cluster:\n");
            for (const auto &chunk: sig_cluster) {
                __debug_print_sigspec(chunk.first);
                log("  Chunk Index: %d\n", chunk.second);
            }
            log("Temp SigSpec: ");
            tmp_sig_spec.sort();
            __debug_print_sigspec(tmp_sig_spec);
            log("\n\n");
        }
    }
